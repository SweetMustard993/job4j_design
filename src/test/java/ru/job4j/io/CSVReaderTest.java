...
class CSVReaderTest {
    ...
    @Test
    void whenColumnsValuesHasQuotes(@TempDir Path folder) throws Exception {
        String data = String.join(
                System.lineSeparator(),
                "name,age,last_name,education,hobbies",
                "Tom,20,Smith,Bachelor,\"\"football, dance\"\"",
                "Jack,25,Johnson,Undergraduate,fantasy",
                "William,30,Brown,Secondary special,\"\"board games,meditation,basketball\"\""
        );
        File file = folder.resolve("source.csv").toFile();
        File target = folder.resolve("target.csv").toFile();
        ArgsName argsName = ArgsName.of(new String[]{
                "-path=" + file.getAbsolutePath(), "-delimiter=,",
                "-out=" + target.getAbsolutePath(), "-filter=education,hobbies,last_name"
        });
        Files.writeString(file.toPath(), data);
        String expected = String.join(
                System.lineSeparator(),
                "education,hobbies,last_name",
                "Bachelor,\"\"football, dance\"\",Smith",
                "Undergraduate,fantasy,Johnson",
                "Secondary special,\"\"board games,meditation,basketball\"\",Brown"
        ).concat(System.lineSeparator());
        CSVReader.handle(argsName);
        assertThat(Files.readString(target.toPath())).isEqualTo(expected);
    }
