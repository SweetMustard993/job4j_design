...
create view show_authors_with_2_or_more_orders as
select count(s.name) as "количество заказов", a.name from students as s
    join orders o on s.id = o.student_id
    join books b on o.book_id = b.id
    join authors a on b.author_id = a.id
    group by (a.name) having count(s.name) >= 2;

select * from show_authors_with_2_or_more_orders;